...
network = regression(loss, optimizer='momentum',
                     ...
                     learning_rate=0.001)
model = tflearn.DNN(network, checkpoint_path='model_googlenet',tensorboard_verbose=0)
model.load('./inception_full_data_my_loss_ckpt-7336')
	...
	path = os.path.join(TRAIN_DIR, theme)
        for j in os.listdir(path):
            ...
            if h in hashes:
                ...
                hashes[h] = [path_new]
Y.sort()
print("id,Mobile_Theme")
count=0
for test_img in (os.listdir(TEST_DIR)):
	path_new = os.path.join(TEST_DIR, test_img)
        h = hashlib.md5(open(path_new, 'rb').read()).hexdigest()
        img = cv2.imread(path_new)
        img = cv2.resize(img, (IMG_SIZE_W, IMG_SIZE_H))
	X=np.array(img).reshape(-1,IMG_SIZE_W,IMG_SIZE_H,3)
	result = model.predict(X)
	if h in hashes and len(hashes[h]) is 1:
		print(test_img,',',labels[hashes[h][0]],sep='')
		if labels[hashes[h][0]] != Y[np.argmax(result,1)[0]]:
			count=count+1
	else:
		print(test_img,',',Y[np.argmax(result,1)[0]],sep='')
print(count)
